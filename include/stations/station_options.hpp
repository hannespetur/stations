...
public:
  BOSS_THREAD_MODE boss_thread_mode = HARD_WORKING_BOSS;
  std::size_t max_queue_size = 2; /** Maximum size of the worker queues, including jobs which are running */
  ...
  std::size_t num_threads = std::thread::hardware_concurrency(); /** Number of threads to use, including the main thread */
