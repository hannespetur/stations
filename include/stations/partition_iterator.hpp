...
get_partition_iterators(BidirectionalIterator first,
                        BidirectionalIterator last,
                        StationOptions const & options)
{
  ...
  if (options.chunk_size == 0)
  {
    ...
    {
      std::size_t const part_size =
        container_original_size / PARTS + (i < container_original_size % PARTS);
  ...
  {
    partition_iterators.reserve(std::distance(first,
                                              last) / options.chunk_size + options.chunk_size + 1);
